...
- **구현 방향**: `cocobackend-blue`(8080)·`cocobackend-green`(8081) 두 개의 systemd 유닛(`deploy/cocobackend-blue.service`·`deploy/cocobackend-green.service`)이 각자 디렉토리(`/home/ubuntu/cocobackend-blue`·`-green`)의 jar를 다른 포트에서 운영하고, `.env`는 두 슬롯이 공유(`/home/ubuntu/cocobackend/.env`, 포트만 유닛별 `Environment=SERVER_PORT`로 분리). nginx(`deploy/nginx-cocobackend.conf`)가 `/etc/nginx/conf.d/cocobackend_upstream.conf`가 가리키는 활성 포트로만 트래픽을 전달하고, `deploy/switch-active.sh`가 이 업스트림 파일을 갱신 후 `nginx -s reload`로 무중단 전환한다.
  - `.github/workflows/deploy.yml`의 빌드·scp·`.env` 작성 단계는 그대로 재사용하고, 마지막 스텝이 `Restart service`(단일 재시작) → `Blue/Green switch`로 교체됨: `active_color` 상태 파일로 비활성 슬롯 판별 → 새 jar 배포 → 재시작 → `/actuator/health` 헬스체크(최대 180초 재시도) 통과 확인 → `switch-active.sh`로 nginx 전환 → `active_color` 갱신 → 이전 슬롯 `systemctl stop`. 헬스체크 실패 시 배포를 실패 처리하고 기존 활성 슬롯은 그대로 유지(자동 롤백 효과).
  - **2026-08-29 실서버 검증 중 발견**: 최초 60초 대기로는 부족해 첫 배포가 헬스체크 타임아웃으로 실패(기존 활성 슬롯은 영향 없이 유지됨, 롤백 설계대로 동작). 실제 기동 로그 확인 결과 `Started CocoBackendApplication`까지 약 76초(서비스 시작 기준 약 82초) 소요 — blue+green 동시 기동 시 메모리 압박(swap 발생 확인)으로 평소보다 느려진 영향으로 추정. 60회 × 3초(최대 180초)로 늘려 재배포 검증 예정.
...
- **TODO (고도화, 2026-09-05)**: Groq 호출이 최대 재시도 소진 후에도 실패하면 지금은 그대로 499 실패 처리됨. 심사·데모 중 Groq 장애가 겹치면 코스 생성이 완전히 막히는 리스크가 있어, AI 없이 Tier 샘플링 결과만으로 최소한의 코스를 반환하는 Degraded Fallback 안전망을 설계 — 공모전 규모에서는 발생 빈도가 낮아 당장 착수 안 함. 상세: [PRD_BACK.md BOQ18](PRD_BACK.md#9-가정-및-미결-open-questions).
...
### CO6. 별점·추천수 기반 순수 알고리즘 코스 추천 (취소, 2026-09-05)
- **설명**: ~~`poi_rating.stars`·`poi_rating.likes` 기반 POI 스코어링 알고리즘으로 여행자 조건(인원 버킷·테마·이동수단·기간·시군구)에 최적화된 Day별 코스를 Groq 없이 생성~~. **2026-09-05 결정: "Groq 완전 제거"라는 로드맵 자체를 취소** — AI(Groq) 기반 코스 생성(CO1)을 앞으로도 핵심 방식으로 영구 유지하기로 확정했다. Groq 의존에 따른 리스크(모델 단종·rate limit·일시 장애)는 알고리즘 전환 대신 [BOQ18 Degraded Fallback 안전망](PRD_BACK.md#9-가정-및-미결-open-questions)(설계만 해두고 미착수)으로 대응하는 방향으로 대체.
  - **stars 기반 Tier 샘플링(v0.2.6)은 취소 대상 아님**: "알고리즘으로 완전 대체하기 위한 중간 단계"가 아니라, CO1 프롬프트 후보 품질을 높이는 보조 신호로서 그 자체로 최종 형태 — 계속 유지·사용.
- **MVP**: ❌ 제외
- **구현 상태**: ❌ (의도적 미구현, 재검토 예정 없음 — Tier 샘플링만 CO1의 일부로 계속 동작)
- **FE 의존**: 없음.
- **가치**: (취소) — 재검토 예정 없음.
...
| ~~v2+~~ | ~~CO6-2~~ | ~~순수 알고리즘 기반 코스 추천 (Groq 완전 제거)~~ — 2026-09-05 취소, AI(Groq) 방향 영구 유지 확정 |
