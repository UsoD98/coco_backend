...
| OQ9 | 기본 추천 코스: v1(공모전 출시)은 Groq AI 생성 유지 확정, 알고리즘 추천은 v2+ 고도화로 연기 (2026-08-16). `§12 로드맵` 참고 | ✅ |
...
> **2026-08-16 결정**: 공모전 출시(v1.0)는 Groq AI 코스 생성을 그대로 유지한다. stars·likes 기반 알고리즘 추천은 출시 후 고도화 단계(v2.0+)로 연기 — 더 나은 설계가 확보되면 그때 붙인다.

| 단계 | 버전 | 방식 | 핵심 변경 |
| --- | --- | --- | --- |
| **Phase 1 — v1.0 (공모전 출시, 확정)** | v0.2~v1.0 | Groq AI 기반 | POI 유형별 할당량 샘플링(stars Tier 샘플링 포함) → LLM 프롬프트 → Day별 코스 반환. 샘플링 소스는 v0.5.0부터 로컬 DB → TourAPI 라이브 조회(Caffeine 캐시 TTL 6h)로 전환 완료 |
| **Phase 2 — v2.0+ 고도화 (보류)** | v2.0+ | stars·likes 가중치 + Groq 보조 | 라이브 조회 결과에서 `poi_rating.stars`·`likes` 상위 우선 선택. LLM은 여전히 일정 조합 담당 |
| **Phase 3 — v3.0+ 고도화 (보류)** | v3.0+ | 순수 알고리즘 추천 | Groq 완전 제거. 인원버킷·테마·이동수단·`poi_rating.stars`·`likes`·사용자 `travel_type`으로 스코어 계산 → 유형별 할당 규칙 엔진으로 Day별 코스 직접 조합 |
...
1. CORS 설정 (INF4)
2. 카카오 OAuth 콜백 (AU4)
3. ~~시군구 목록·데이터 보유 플래그 API (PO4)~~ (취소)
