...
5. ~~무중단 배포(blue/green) 전환~~ — 레포 측 구현 완료(BOQ16), 서버 최초 1회 설정(`deploy/BLUEGREEN_SETUP.md`)만 수동 적용 대기
...
- **BOQ1. 인원 버킷 정의** — ✅ **대상 소멸 (2026-09-05 문서 정리)**: PO2의 인원 버킷(`peopleCount`) 필터 자체가 BOQ14(2026-08-08/08-22)로 스코프아웃 확정되면서 매핑을 결정할 대상이 사라짐. `peopleCount`는 CO1(코스 생성) 요청에서만 그대로 정수로 수신·사용.
...
- **BOQ12. `stars`·`likes` 데이터 수집 방법** — 부분 확정: `likes`는 PO5 좋아요 토글 API(v0.2.6)로 앱 내 수집. `stars`는 초기 285개 이후 추가 자동 수집 방법 미확정. v0.5.0부터 저장 위치는 `poi_rating` 테이블.
  - **2026-09-05 조사 결과 — 자동화 경로 3개 다 막힘, 당장 착수 안 함**: 경북 전체 라이브 후보(areaCode=35, 7개 콘텐츠타입) 재조회 결과 총 1,624건으로 초기 285건(약 17.5%) 대비 갭이 큼을 확인. 무료 자동 수집 경로를 조사했으나:
    - **구글 Places API**: 공식 `rating` 필드가 있으나 결제 계정(카드) 등록이 필수이고, `rating`은 "Atmosphere Data"로 분류돼 기본 요청보다 추가 과금이 붙는 유료 항목 — 예산 제약상 배제.
    - **네이버 지도**: 프론트가 쓰는 비공식 검색 엔드포인트(`map.naver.com/p/api/search/allSearch`)를 실제로 호출해보니 정상 파라미터를 갖춰도 `ncaptcha`(캡차) 응답으로 차단됨 — 자동화 요청을 적극적으로 탐지하고 있어 우회 시도는 하지 않기로 함.
    - **카카오**: 공식 로컬 검색 API(`dapi.kakao.com`, 무료·카드 불필요)는 검색·매칭엔 쓸 수 있지만 응답에 별점 필드 자체가 없음 — 확인 결과 카카오 개발자 API로는 별점 정보를 제공하지 않음.
    - 결론: 세 경로 모두 (비용/차단/데이터 부재로) 막혀 있어 현재는 완전 자동화된 무료 수집 방법이 없음. 재검토 시점·방향은 미정 — 다음에 다시 볼 것.
